...
def process_inputs(audio_path: str, 
    ...
    with st.spinner("Generating IPA variants..."):
        ...
        for i, model in enumerate(model_paths[:3]):
            ipa = generate_ipa(load_model(model), original_text)
            if ipa:
        ...
        for i, model in enumerate(model_paths[:3]):
            ipa = generate_ipa(load_model(model), transcribed_text)
            if ipa:
    ...
    with st.spinner("Evaluating transcriptions..."):
        evaluation = evaluate_transcriptions(
            load_model(model_paths[3]),  # Evaluation model
            original_text,
            transcribed_text,
            original_ipas,
            transcribed_ipas
